...
public class TracingConfig {
	...
	WebFilter tracingFilter() {
		return (exchange, chain) -> {
			final String traceId = Objects.isNull(exchange.getRequest().getHeaders().getFirst("X-Trace-Id"))
					? java.util.UUID.randomUUID().toString()
					: exchange.getRequest().getHeaders().getFirst("X-Trace-Id");
			exchange.getAttributes().put("X-Trace-Id", traceId);

			return chain.filter(exchange).doFinally(signal -> {
				log.info("Request: path={}, method={}, status={}, traceId={}, userId={}",
						exchange.getRequest().getPath(), exchange.getRequest().getMethod(),
						exchange.getResponse().getStatusCode(), traceId,
